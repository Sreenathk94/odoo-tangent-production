...
class AttendanceClaim(Controller):
    ...
    if kwargs.get('employee_id') and kwargs.get('date'):
        ...
        sterday_india = india_tz.localize(sterday)
        sterday_dubai = dubai_tz.localize(sterday)

        for attendance in request.env['hr.attendance'].search([
            ('fetch_date', '=', sterday),
            ('employee_id', '=', employee_id.id)
        ]).filtered(lambda a: a.actual_hours < request.env.company.attend_work_hrs):
            data_to_load_html_template.append([
                'First Check-in & Last Check-out',
                (attendance.check_in).strftime("%d-%m-%Y %H:%M:%S"),
                (attendance.check_out).strftime("%d-%m-%Y %H:%M:%S"),
                self.float_to_time(attendance.worked_hours), ' '
            ])
            lunch_break = time(13, 0) <= attendance.check_in.time() <= time(14, 0) or time(13,
                         0) <= attendance.check_out.time() <= time(14, 0)
            if lunch_break:
                non_counted = timedelta(days=0)
                counted = timedelta(days=0)
                data_to_load_html_template.append(['Breaks', ' ', ' ', 'Counted', 'Non-Counted'])
                ...
                for line in attendance.line_ids:
                    ...
                    j += 1
                data_to_load_html_template.append(['Total Breaks', ' ', ' ', str(counted), ' '])
                wk_hr = timedelta(hours=attendance.worked_hours)
                bk_hr = counted if lunch_break and counted > timedelta(hours=1) else counted + timedelta(hours=1)
                net_time = wk_hr - bk_hr
                data_to_load_html_template.append([
                    'Net total time inside the office (' + str(
                        self.float_to_time(attendance.worked_hours)) + ' - ' + str(counted) + ')',
                    ' ', ' ', str(net_time), ' '])
