...
class tgLeaveRegister(models.TransientModel):
    ...
    def print_report(self):
        ...
        header_format = xlwt.easyxf(
            'font: bold True, name Calibri; align: horiz center; pattern: pattern solid, fore-colour grey25; borders: left thin, right thin, top thin, bottom thin;'
        )
        days_format = xlwt.easyxf(
            'font: name Calibri; align: horiz center; borders: left thin, right thin, top thin, bottom thin;'
        )
        bold_left_format = xlwt.easyxf(
        ...
        highlight_format = xlwt.easyxf(
            'pattern: pattern solid, fore-colour pink; font: name Calibri; align: horiz center; borders: left thin, right thin, top thin, bottom thin;'
        )

        # Set column widths for readability
        sheet.col(0).width = int(40 * 260)
        for col_idx in range(1, 10):
            sheet.col(col_idx).width = int(17 * 260)

        # Add header row with background shade
        col1 = 1
        col2 = 3
        for lt in self.env['hr.leave.type'].search([('id', '=', self.leave_type_ids.ids)], order='id asc'):
            sheet.write_merge(0, 0, col1, col2, lt.name, header_format)
            sheet.write(1, col1, 'Days', header_format)
            col1 += 1
            sheet.write(1, col1, 'Half Day', header_format)
            col1 += 1
            sheet.write(1, col1, 'Permission (Hrs)', header_format)
            col1 += 1
            col2 += 3

        # Populate data rows with conditional formatting
        row = 2
        for emp in self.employee_ids:
            col1 = 1
            col2 = 3
            sheet.write(row, 0, emp.name, bold_left_format)
            ...
            for leave_type in self.env['hr.leave.type'].search([('id', '=', self.leave_type_ids.ids)], order='id asc'):
                ...
                # Write leave data with conditional formatting
                sheet.write(row, col1, leave_full, highlight_format if leave_full > 0 else days_format)
                col1 += 1
                sheet.write(row, col1, leave_half, highlight_format if leave_half > 0 else days_format)
                col1 += 1
                sheet.write(row, col1, leave_permission, highlight_format if leave_permission > 0 else days_format)
                col1 += 1
                col2 += 3
            row += 1
        ...
        return {
            ...
            'url': 'web/content/?model=tg.leave.register&field=excel_file&download=true&id=%s&filename=%s' % (
                self.id, 'Employee_Leave_Report.xls'),
            'target': 'new',
        }
